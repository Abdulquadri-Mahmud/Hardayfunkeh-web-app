...
const cartSlice = createSlice({
    ...
    reducers : {
        addToCart : (state, action) => {
            const { productID, quantity, productName, stock, productPrice, size, productImage, productSize} = action.payload;
            const findProductsID = (state.items).findIndex(item => item.productID === productID);
            if (findProductsID >= 0) {
                // state.items[findProductsID].quantity += quantity;
            }
            else{
                state.items.push({productID, quantity, stock, productName, productImage, productPrice, size, productSize});
